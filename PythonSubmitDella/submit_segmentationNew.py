...
# 
'''
The code is adapted from the original submit_segmention.py for the new low mag design with only IR camera

This code is just for submitting jobs to Princeton Della, and the only change is writing the slurm job submit_piplineNew.py instead of submit_pipline.py
...
def submitScript(master=None):
    ...
    commandList.append("python submit_pipelineNew.py --folder " + fullPath)
...
# bind enter key and button
    print('''
        This is the submission script for running segmentation.
