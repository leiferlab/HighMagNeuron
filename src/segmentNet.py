...
This is for segmenting neuron from an aligned volume
@author: xinweiy
"""
...
def generate_kernel(kernel_size, z_diff):
    # kernel for array [z, x, y]
    #sigma_x = sigma_y = 1.6
    sigma_x = sigma_y = 1.0# 1.2
    sigma_z = 0.015 / z_diff   # value set manually 0.055
    z_m = np.arange(-(kernel_size[0] - 1) // 2, (kernel_size[0] - 1) // 2 + 1)
    x_m = np.arange(-(kernel_size[1] - 1) // 2, (kernel_size[1] - 1) // 2 + 1)
    y_m = np.arange(-(kernel_size[2] - 1) // 2, (kernel_size[2] - 1) // 2 + 1)
    zv, xv, yv = np.meshgrid(z_m, x_m, y_m, indexing='ij')
    kernel = np.exp(-0.5 * ((xv / sigma_x) ** 2 + (yv / sigma_y) ** 2))
    kernel *= (1 + (zv / sigma_z) ** 2) ** (-0.5)
    return kernel
...
class Detect_From_Deconv(object):
    ...
    def get_gery_mask(self, worm, fac=3):
        ...
        #Multi_Slice_Viewer(np.concatenate((worm, worm_diff), axis=2))
        ...
        grey_mask = remove_small_objects(grey_mask, min_size=15, in_place=True)
        #Multi_Slice_Viewer(np.concatenate((grey_mask, grey_mask_new), axis=2))
        #grey_mask_new = grey_mask
        return grey_mask
    ...
    def detect_neuron_hessian(self, worm, z_diff, show=1, worm_green=None):
        ...
        worm = ndi.uniform_filter(worm, size=[1, 3, 3])
        # get rid of the background(important)
        grey_mask = self.get_gery_mask(worm)

        if worm_green is not None:
            # also use GCamp, together with RFP
        ...
        mask_n = H_eig > 0.1
        area_thd = 6
        mask_n = remove_small_objects(mask_n, min_size=area_thd, in_place=True)
        #Multi_Slice_Viewer(mask_n)
        label_n = ndi.label(mask_n)[0]

        if label_n.max() > 800:
            # too many neurons here.
            return None
