...
MIN_TIME_STR = "--time=12:02:00"  #minimum time string for use on short queue
...
def check_input(commandList,fullPath,totalRuns,nCheck,nNeurons,email_flag = False):
    ...
    time_estimate=int(np.round(nCheck*totalRuns*.2/60/nNeurons)) #Changed/divided by nNeurons. Test if this is enought time for checks- KH 1/6/20
    qString_check = "--time="+ str(np.max((time_estimate,240)))   
    ...
    if socket.gethostname()=='tigressdata.princeton.edu' or socket.gethostname()=='tigressdata2.princeton.edu':
        ...
        with open(fileName,'a') as f:
            for command in commandList:
                ...
                f.write('\r\n')
            s = '775'
            os.chmod(fileName, int(s, base=8)) #
    else:
...
def make_ouputfolder(client,fullPath):
    ...
    if socket.gethostname()=='tigressdata.princeton.edu':
        ...
        try:
            ...
            ftp.mkdir(outputFilePath)
            time.sleep(1)
            ftp.chmod(outputFilePath,0o6775)
            ftp.close()
